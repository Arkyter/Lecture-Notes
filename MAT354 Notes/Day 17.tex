...
\begin{lemma}[Schwarz Lemma, \S 8.2.1]
    ...
    \begin{enumerate}[(i)]
        ...
        \item if $\abs{f(z_0)} = \abs{z_0}$ for some $z_0 \in \DD \setminus \{0\}$, then $f$ is a rotation (meaning $f(z) = e^{i\theta} z$ for some $\theta \in \RR$, or alternatively written $f(z) = cz$ with $c \in \CC$, $\abs{c} = 1$),
