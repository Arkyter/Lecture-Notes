...
\begin{proof}
    ...
    by the residue formula, since we may observe the latter integrand is holomorphic. For $w_k$, we have that $f(z) = (z - w_k)^{-n_k} h_k(z)$, and $f'(z) = -n_k(z - w_k)^{-n_k - 1} h_k(z) + (z - w_k)^{-n_k} h_k'(z)$, so from the above, we obtain that their integrals are simply $-2\pi n_k$, and so this yields our sum as desired.
\end{proof}