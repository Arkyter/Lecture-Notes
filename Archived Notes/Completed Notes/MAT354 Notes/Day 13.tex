...
\begin{proof}
    ...
    Since $z_j$ is a zero, we may find an open disc $D_r(z_j)$, a holomorphic non-vanishing $g_j$ on $D_r(z_j)$, and a unique $n_j \in \NN$ such that
    \begin{align*}
        f(z) &= (z - z_j)^{n_j} g_j(z), \\
        f'(z) &= n_j (z - z_j)^{n_j - 1} g_j(z) + (z - z_j)^{n_j} g_j'(z),
    \end{align*}
    for all $z \in D_r(z_j)$. Thus, we have
    \[ \int_{-C_{z_j,\eps}} \frac{f'(z)}{f(z)} \, dz = \int_{-C_{z_j, \eps}} \frac{n_j}{z - z_j} + \int_{-C_{z_j,\eps}} \frac{g_j'(z)}{g_j(z)} = 2\pi n_j \]
    by the residue formula, since we may observe the latter integrand is holomorphic (as $g \neq 0$ in the interior of the circle). For $w_k$, we have that $f(z) = (z - w_k)^{-n_k} h_k(z)$, and $f'(z) = -n_k(z - w_k)^{-n_k - 1} h_k(z) + (z - w_k)^{-n_k} h_k'(z)$, so from the above, we obtain that their integrals are simply $-2\pi n_k$, and so this yields our sum as desired.
\end{proof}