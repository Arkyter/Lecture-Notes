...
\begin{question}
	...
	This turns out to be equivalent to \( ||f - g||_{L^{2}} = 0 \), which is fun. After modding out by the self-orthogonal vectors, we have that \( (\cdot, \cdot) \) is an inner product. Fabio defined it in class as \( f \) and \( g \) differing only on a set with measure zero, but I like this definition more because it is more direct in my humble opinion. 
\end{question}
