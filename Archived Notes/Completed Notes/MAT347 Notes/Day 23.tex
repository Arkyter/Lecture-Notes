...
\begin{definition}
    A division ring is a field in which $\cdot$ isn't necessarily commutative. More formally, it's a ring $R$ in which, if $x \neq 0$, then there exists $y$ such that $xy = yx = 1$.
\end{definition}
...
\end{proof}
\noindent Take $J \supset I_\infty$; there are $2^{2^\omega}$ because $\abs{\beta \NN} = 2^{2^{\omega}}$ (the set of ultrafilters on $\NN$ coincides with the Stone--\v{C}ech compactification of $\NN$). We claim that $\ell^\infty / J \cong \RR$.
\begin{proof}
