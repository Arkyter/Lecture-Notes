...
Now, let's solve the Schr\"odinger equation for fun. Recall that this is \( u_{t} = i u_{x x} \). ``It's like the heat, with \( k \to i \).'' He then shouted loudly ``\( k \) STILL HAS TO BE REAL!!'' which scared me. Nevertheless, if we just naively plug in to the formula for the heat solution, we get a solution to the Schr\"odinger equation
