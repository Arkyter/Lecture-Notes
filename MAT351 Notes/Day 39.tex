 % ! TeX root = ../mat351.tex
\section{Day 39: Properties of Fourier Transform (Mar.\ 18, 2026)}
...
\begin{definition}[Schwartz Space]
	\( S(\mathbb{R}^{d}) \) is the Schwartz space, which consists of all \( f \colon \mathbb{R}^{d} \to \mathbb{C} \) such that
	\[ \sup_{x \in \mathbb{R}^{d}} \left| |x^{\alpha}| D^{\beta} f(x)| \right| < c_{\alpha, \beta}. \]
	So the Schwartz space is smooth functions which are decaying faster than any polynomial.
...
\begin{remark}\relax
	...
	\begin{enumerate}
		...
		\item So long as our function is Schwartz, the algebra works out. In analogy, imagine the Fourier series, where differentiating multiplies by the frequency.
