...
\end{definition}
\noindent Note that we define $D_x^{(\ell)}$ to be the collcetions of derivatives of order $\ell$. Next class, we will see that $\hat \SF \colon \mathcal S \to \mathcal S$ (which you can read about in Stein \S5.1.3!). To wrap up today's lecture, we give a few examples of Schwarz functions on $\RR^d$.
\begin{enumerate}[(i)]
